...
namespace UnityEditor {
	public static class AudioUtility {
		...
		public static Texture2D GetWaveForm(AudioClip clip , int channel , float width , float height) {
			...
			return texture;*/
            return AssetPreview.GetAssetPreview(clip);
		}
		...
		public static AudioType GetPlatformConversionType(AudioType inType , BuildTargetGroup targetGroup, AudioCompressionFormat format)//  AudioImporterFormat format)
        {
			Assembly unityEditorAssembly = typeof(AudioImporter).Assembly;
			Type audioUtilClass = unityEditorAssembly.GetType("UnityEditor.AudioUtil");
			MethodInfo method = audioUtilClass.GetMethod(
